...
- **This repo is one of ten identical ones.** The shared config files, scripts
  ...
  find months later. This repo is the family's documented exception wherever
  playwright-core ships beside the bundle; `SHARED_WITH_EXCEPTIONS` in that
  script is the list, and anything not on it is a regression rather than a
  decision. Anything under `crate/` is outside the check on purpose: the
  crates stand on their own.
