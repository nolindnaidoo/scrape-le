//! The Zed side of Scrape-LE.
//!
//! This crate holds no extraction logic. It resolves `scrape-le-mcp` from npm and
//! hands Zed a command that launches it over stdio — the same server the VSIX
//! bundles and the same one `npx scrape-le-mcp` runs. Keeping it a launcher is
...
const PACKAGE_NAME: &str = "scrape-le-mcp";
const SERVER_PATH: &str = "node_modules/scrape-le-mcp/server.js";

struct ScrapeLEExtension;
...
impl zed::Extension for ScrapeLEExtension {
    fn new() -> Self {
        Self
    }
...
zed::register_extension!(ScrapeLEExtension);
