...
send({
	...
	params: {
		...
		arguments: {"content": "User-agent: *\nDisallow: /admin\nCrawl-delay: 5\nSitemap: https://example.com/sitemap.xml", "path": "/admin"},
	},
});
...
setTimeout(() => {
	...
	if (init?.serverInfo?.name !== 'scrape-le') {
	...
	if (call?.isError !== false) {
		fail('analyze_robots_txt reported an error', JSON.stringify(call));
	}
	const data = call?.structuredContent?.data;
	// Asserting the verdict, not just "it responded": a server that reported
	// everything as allowed would otherwise pass, and that is the dangerous
	// direction to be wrong in.
	if (data?.allowsCrawling !== false) {
		fail(
			'analyze_robots_txt allowed a path robots.txt disallows',
			JSON.stringify(data),
		);
	}
	if (data?.crawlDelay !== 5) {
		fail('analyze_robots_txt lost the crawl delay', JSON.stringify(data));
	}
	if (!(data?.sitemaps ?? []).includes('https://example.com/sitemap.xml')) {
		fail('analyze_robots_txt lost the sitemap', JSON.stringify(data));
	}
